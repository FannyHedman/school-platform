...
const RequestedSchedule = () => {
    ...
    const handleWeekToggle = (weekId) => {
        setSelectedDays((prev) => {
            ...
            for (const day of weeks.find((week) => week.week_id === weekId)
                ?.day) {
                updatedWeek[day] = allSelected
            }
            ...
            for (const [weekId, weekData] of Object.entries(selectedDays)) {
                for (const [dayId, attending] of Object.entries(weekData)) {
                    ...
                    const response = await axios.put(
                        `http://localhost:8800/week_day_association/${childId}`,
                        data
                    )
                    console.log(
                        `Attendance for Child ${childId} Week ${weekId}, Day ${dayId} submitted successfully:`,
                        response.data
                    )
                }
            }
    ...
    return (
        <Container className="container">
            ...
            <SubmitButton onClick={handleSubmit} disabled={showModal}>
                Submit
            </SubmitButton>
            {showModal && (
                <ConfirmationModalReq handleClose={() => setShowModal(false)} />
            )}
        </Container>
    )
}
